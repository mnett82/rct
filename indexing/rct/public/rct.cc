...
int RCT::partialQuickSort(int howMany, float* distList, int* indexList,
  ...
  numFound =
      partialQuickSort(howMany, distList, indexList, rangeFirst, pivotLoc - 1);
  ...
  return numFound + 1 + partialQuickSort(howMany - numFound - 1, distList,
                                         indexList, rangeFirst + numFound + 1,
                                         rangeLast);
}
  ...
  for (int L = levels - 1; L > 0; --L) {
    ...
    for (int i = 0; i < levelSetSizeList[L]; ++i) {
      ...
      for (int j = 0; j < numChildren; ++j) {
        ...
        // Find the nearest-neighbor's distance of the child at the parent level
        // L.
        setNewQuery(child);
        doFindNearest(1, L);
